...
static QScriptValue qtscript_QClipboard_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 1) {
        QClipboard::Mode _q_arg0 = qscriptvalue_cast<QClipboard::Mode>(context->argument(0));
        QMimeData* _q_result = const_cast<QMimeData*>(_q_self->mimeData(_q_arg0));
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
...
QScriptValue qtscript_create_QClipboard_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 15; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QClipboard_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QClipboard_static_call, proto, function_lengths[0]);
