...
QScriptValue qtscript_create_QTextFrame_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 11; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QTextFrame_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QTextFrame_static_call, proto, function_lengths[0]);
