...
static QScriptValue qtscript_QGraphicsLayout_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (!_q_self) {
        return context->throwError(QScriptContext::TypeError,
            ...
            .arg(qtscript_QGraphicsLayout_function_names[_id+1]));
    }
    ...
    return qtscript_QGraphicsLayout_throw_ambiguity_error_helper(context,
        qtscript_QGraphicsLayout_function_names[_id+1],
        qtscript_QGraphicsLayout_function_signatures[_id+1]);
}
...
QScriptValue qtscript_create_QGraphicsLayout_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 9; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QGraphicsLayout_prototype_call, function_lengths[i+1]);
        fun.setData(QScriptValue(engine, uint(0xBABE0000 + i)));
        proto.setProperty(QString::fromLatin1(qtscript_QGraphicsLayout_function_names[i+1]),
            fun, QScriptValue::SkipInEnumeration);
    }

    engine->setDefaultPrototype(qMetaTypeId<QGraphicsLayout*>(), proto);

    QScriptValue ctor = engine->newFunction(qtscript_QGraphicsLayout_static_call, proto, function_lengths[0]);
