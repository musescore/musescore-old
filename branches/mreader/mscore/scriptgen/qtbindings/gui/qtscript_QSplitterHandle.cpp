...
QScriptValue qtscript_create_QSplitterHandle_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 6; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSplitterHandle_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSplitterHandle_static_call, proto, function_lengths[0]);
