...
struct TextSegment {
      ...
      TextSegment(const Font& f, qreal _x, qreal _y) : font(f), x(_x), y(_y), select(false) {}
      TextSegment(const QString&, const Font&, qreal x, qreal y);
      void set(const QString&, const Font&, qreal x, qreal y);
...
class Harmony : public Text {
      ...
      QList<Font> fontList;               // temp values used in render()
