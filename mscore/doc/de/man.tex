...
\chapter{Einführung}
   \section{Was ist \M?}
      ...
      \startitemize[packed]
            ...
            \item Noten können einfach und schnell nur über die Tastatur
                  eingegeben werden
...
mscore
   ...
   \startitemize[packed]
      \item einer Leiste mit pulldown Menüs
  ...
  \subsection{Neue Partitur erzeugen}
      ...
      mit dem gewähl\-ten Template angezeigt, in unserem Fall also zwei
...
\subsection{Drucken}
      ...
      Durch anclicken des Druker Knopfes wird der Druck-Dialog des
      KDE Systems gestartet. \M\ erzeugt PostScript Druckerdaten,
      die KDE zum Drucker schickt oder in eine Datei umleitet.
      Das KDE Druckmenü erlaubt auch die direkte Erzeugung von
      PDF-Dateien.
...
\stoptyping
      ...
      \M\ schreibt/liest im Hintergrund zwei weitere Dateien:
            ...
            \NC \type{~/.mscorePrj} \NC enthält die zuletzt
   ...
   \section{Menüs}
      ...
      \startpacked
            \Opt{Instruments}
                  öffnet den Instrumenten-Dialog. Dort kön\-nen
      ...
      \subsection{Notes Menu}
         ...
         \startpacked
            ...
            \Opt{Add Pitch}
                  zeigt ein weiteres Untermenü von Notenwerten,
      ...
      \startpacked
            ...
            \Opt{Set Normal Staff Distances} stellt manuell veränderte
...
%----------------------------------------------------------
   ...
   \section{Noten Eingabe}
      ...
      nächste Note durch Click gesetzt werden kann. Shift+Click fügt
      eine Note hinzu, so das ein Akkord entsteht.
      ...
      ein Vorzeichen aus der Toolbox anzuclicken. Die gewählte Note
      bekommt nun das Vorzeichen und \M\ verändert die Tonhöhe der
      Note gemäß dem gewählten Vorzeichen. Auf diese Art läßt sich
...
\section{Navigation}
      ...
      \startpacked
            \Input{Right}
                  geht zur nächsten Noten\par
            \Input{Left}
                  geht eine Note zurück\par
            \Input{Alt+Up}
                  selektiert höhere Note im Akkord oder niedrigste
      ...
      beide Teile.
            ...
            \startitemize[packed]
                  ...
                  \item einen aktuellen X11 Server mit freetype2 Support
                        und Render Extension (für antialiased Bildschirmfonts).
                  \item gcc 3.3 oder gcc 3.4
                  \item Empfohlen: kde3 (\M\ benutzt standardmäßig den
                        KDE Printer). Ohne KDE müssen sie im Untermenü "Einstellungen"
                        das Druck-Kommando konfigurieren.
            \stopitemize

   \section{Compilieren}
      Das Übersetzen von \M\ geschieht durch die üblichen Befehle
      \type{configure} und \type{make}.

   \subsection{configure}
      Hinweis: Configure funktioniert nur, wenn es von einer X11-Konsole
            aufgerufen wird.

\starttyping
cd mscore-x.x.x}{./configure --prefix=/usr
\stoptyping
      ...
      Wenn sie den GNU Compiler $\ge$ 3.4 installiert haben, dann
      können sie die Übersetzung durch die Verwendung von precompilierten
      Include-Dateien verdreifachen. Konfigurieren sie \M\ dazu
      mit der Option:
...
\stoptyping
      ...
      Unglücklicherweise wird der benötigte Notensatzfont nicht automatisch
      installiert.

      Unter KDE gehen sie dazu wie folgt vor:

      \startitemize[packed]
            \item Starte das KDE Kontrollcenter
            \item gehe nach "System Administration"
            \item start "Font Installer"
            \item wähle den "Aministrator Mode"
            \item wähle das Installationsverzeichnis (\type{TTF})
            \item drücke "Add" und wähle den Font \type{mscore20.ttf} aus
            \item drücke "Apply"
            \stopitemize

      Für Mandrake Benutzer gibt es das Programm \type{drakfont} zur
      Fontinstallation.

      Die "harte" Tour zur Installation von Fonts auf X11-Systemen geht
      wie folgt:

      \startitemize[packed]
            \item Erzeuge das Verzeichnis \type{/usr/share/fonts/truetype}
            \item Kopiere \type{mscore20.ttf} nach \type{/usr/share/fonts/truetype}
            \item Verwende \type{ttmkfdir} um die Datei \type{fonts.scale}
                  zu erzeugen:
\starttyping
ttmkfdir > fonts.scale
\stoptyping

            \item use mkfontdir to create fonts.dir
\starttyping
mkfontdir
\stoptyping

            \item tell the x11 server your new font
\starttyping
xset fp rehash
\stoptyping

            \item Ergänze den Font-Suchpfad des X11-Servers um
                  \type{/usr/share/fonts/truetype} so das er den Font auch
                  nach dem nächsten Reboot wiederfindet.
            \stopitemize

      Übrigens muß der Font zum Drucken nicht nocht einmal installiert werden,
      da \M\ die Fontdaten in den erzeugten Druckdatenstrom
      einbettet.
...
\chapter{Tastaturabkürzungen}
      ...
      \Sc{Alt+Up}\crlf selektiert höhere Note im Akkord oder niedrigste
                  Note der höheren Notenzeile\par
      \Sc{Alt+Down}\crlf selektiert niedrigere Note im Akkord oder die höchste
                    Note der nächsten Notenzeile.\par
      \Sc{Alt+Ctrl+Up}  \crlf selektiert die höchste Note im Akkord\par
