...
Catalog::Catalog(XRef *xrefA) {
  ...
  obj.free();
  pages = (PPage **)gmallocn(pagesSize, sizeof(PPage *));
...
int Catalog::readPageTree(Dict *pagesDict, PageAttrs *attrs, int start,
  ...
  for (i = 0; i < kids.arrayGetLength(); ++i) {
    ...
    if (kid.isDict("Page")) {
      ...
      page = new PPage(xref, start+1, kid.getDict(), kidRef.getRef(), attrs2, getForm());
      if (!page->isOk()) {
	++start;
	goto err3;
      }
      if (start >= pagesSize) {
	pagesSize += 32;
	pages = (PPage **)greallocn(pages, pagesSize, sizeof(PPage *));
