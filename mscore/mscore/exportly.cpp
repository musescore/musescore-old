...
bool ExportLy::findSpecificMatchInMeasure(int tick, Staff* stf, Measure* m, int strack, int etrack)
{
  ...
  for (int st = strack; st < etrack; ++st) 
      ...
      for (Segment* seg = m->first(); seg; seg = seg->next()) 
	{
	  ...
	  if (!el)
	    continue;
	  if (!(el->isChordRest()) && ((el->tick() == tick) || ((el->tick() + el->tickLen()) == tick)))
	    {
	      printf("funnet uten chordorrest\n");
	    }
	
	  if ((el->isChordRest() && el->staff() == stf) && ((el->tick() == tick) || ((el->tick() + el->tickLen()) == tick)))
	    {
	      anker.anchor=el;
	      found=true;
	      anker.tick=tick;
...
void ExportLy::buildInstructionList(Measure* m, bool dopart, Part* p, int strack, int etrack)
{
  ...
  for (ciElement ci = m->el()->begin(); ci != m->el()->end(); ++ci) 
	...
	case STAFF_TEXT:
	  ...
	  if (!found) printf("element ikke forankret: %d\n", instruction->type());
	  break;
	default:
