...
class Fluid : public Synti {
      static const int SILENT_BLOCKS = 32*5;
      int silentBlocks;

      static SFont* sf;             // the loaded soundfont
      ...
      float _tuning[128];           // the pitch of every key, in cents
   ...
   public:
      ...
      virtual void play(const SeqEvent&);
      ...
      virtual float masterTuning() const     { return _masterTuning; }
      ...
      Voice* alloc_voice(Sample* sample, int chan, int key, int vel, float vt);
      void free_voice_by_kill();

      void set_chorus(int nr, float level, float speed, float depth_ms, int type);
      void set_reverb(float roomsize, float damping, float width, float level);
      void program_reset();
      ...
      float getPitch(int k) const   { return _tuning[k]; }
      float ct2hz_real(float cents)  { return powf(2.0f, (cents - 6900.0f) / 1200.0f) * _masterTuning; }

      float act2hz(float c)     { return 8.176 * pow(2.0, (float) c / 1200.0); }
...
class Generator {
   public:
      ...
      float val;          /**< The nominal value           */
      float mod;          /**< Change by modulators        */
      float nrpn;         /**< Change by NRPN messages     */

      void set_mod(float _val)  { mod  = _val; }
      void set_nrpn(float _val) { nrpn = _val; }
      };
...
struct Mod
      ...
      void set_amount(float val)               { amount = val;  }
...
#define FLUID_FRACT_MAX       4294967296.0
...
#define fluid_phase_float(_x) \
  ((float)((_x).index()) + ((float)((_x).fract()) / FLUID_FRACT_MAX))
...
#define fluid_phase_steps(_phase,_idx,_incr) \
  (int)(((float)(_idx) - fluid_phase_float(_phase)) / (float)_incr)
