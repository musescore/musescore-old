...
static QString qtscript_QSessionManager_RestartHint_toStringHelper(QSessionManager::RestartHint value)
{
    if ((value >= QSessionManager::RestartIfRunning) && (value <= QSessionManager::RestartNever))
        return qtscript_QSessionManager_RestartHint_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QSessionManager_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 16; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSessionManager_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSessionManager_static_call, proto, function_lengths[0]);
