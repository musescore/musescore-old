...
static QString qtscript_QAbstractItemDelegate_EndEditHint_toStringHelper(QAbstractItemDelegate::EndEditHint value)
{
    if ((value >= QAbstractItemDelegate::NoHint) && (value <= QAbstractItemDelegate::RevertModelCache))
        return qtscript_QAbstractItemDelegate_EndEditHint_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QAbstractItemDelegate_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 8; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QAbstractItemDelegate_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QAbstractItemDelegate_static_call, proto, function_lengths[0]);
