...
{
    if ((value >= QDoubleValidator::StandardNotation) && (value <= QDoubleValidator::ScientificNotation))
        return qtscript_QDoubleValidator_Notation_keys[static_cast<int>(value)-static_cast<int>(QDoubleValidator::StandardNotation)];
    return QString();
}
...
static QScriptValue qtscript_construct_QDoubleValidator_Notation(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QDoubleValidator::StandardNotation) && (arg <= QDoubleValidator::ScientificNotation))
...
QScriptValue qtscript_create_QDoubleValidator_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 2; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QDoubleValidator_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QDoubleValidator_static_call, proto, function_lengths[0]);
