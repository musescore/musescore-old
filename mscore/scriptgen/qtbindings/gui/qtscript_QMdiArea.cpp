...
static QString qtscript_QMdiArea_AreaOption_toStringHelper(QMdiArea::AreaOption value)
{
    if ((value >= QMdiArea::DontMaximizeSubWindowOnActivation) && (value <= QMdiArea::DontMaximizeSubWindowOnActivation))
        return qtscript_QMdiArea_AreaOption_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QMdiArea_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 8; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QMdiArea_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QMdiArea_static_call, proto, function_lengths[0]);
