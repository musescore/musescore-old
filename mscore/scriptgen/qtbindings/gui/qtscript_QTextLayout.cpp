...
static QString qtscript_QTextLayout_CursorMode_toStringHelper(QTextLayout::CursorMode value)
{
    if ((value >= QTextLayout::SkipCharacters) && (value <= QTextLayout::SkipWords))
        return qtscript_QTextLayout_CursorMode_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QTextLayout_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 33; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QTextLayout_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QTextLayout_static_call, proto, function_lengths[0]);
