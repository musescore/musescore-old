...
//  $Id: system.cpp 3667 2010-11-03 07:58:42Z wschweer $
//
...
void System::layout(qreal xo1)
      ...
      for (int staffIdx = 0; staffIdx < nstaves; ++staffIdx) {
            ...
            if (ss->instrumentName && !ss->instrumentName->isEmpty()) {
                  ...
                  qreal w = ss->instrumentName->width() + point(instrumentNameOffset);
                  if (w > xoff2)
                        xoff2 = w;
                  }
            }
      ...
      foreach (Part* p, *score()->parts()) {
            ...
            if (s->instrumentName && !s->instrumentName->isEmpty()) {
                  qreal d  = point(instrumentNameOffset) + s->instrumentName->bbox().width();
...
void System::layout2()
      ...
      foreach(Part* p, *score()->parts()) {
            ...
            if (s->instrumentName && !s->instrumentName->isEmpty()) {
                  ...
                  //
                  qreal y1 = s->bbox().top();
                  qreal y2 = staff(staffIdx + nstaves - 1)->bbox().bottom();
                  qreal y  = y1 + (y2 - y1) * .5 - s->instrumentName->bbox().height() * .5;
