...
//  $Id: durationtype.cpp 3151 2010-06-10 11:15:15Z lasconic $
//
...
void TimeDuration::setVal(int ticks)
      {
      if (ticks == 0)
            _val = V_MEASURE;
      else {
            TimeDuration dt;
            for (int i = 0; i < TimeDuration::V_ZERO; ++i) {
                  dt.setType(TimeDuration::DurationType(i));
                  int t = dt.ticks();
                  if (ticks / t) {
                        int remain = ticks % t;
                        if ((t - remain) < (t/4)) {
...
QString TimeDuration::name() const
      {
      switch(_val) {
...
int TimeDuration::headType() const
      {
      int headType = 0;
      switch(_val) {
            case V_256TH:
...
bool TimeDuration::hasStem() const
      {
      switch(_val) {
            case V_256TH:
            case V_128TH:
...
TimeDuration::TimeDuration(const QString& s)
      {
      setType(s);
      _dots = 0;
      }
...
void TimeDuration::setType(const QString& s)
      {
      if (s == "quarter")
            _val = V_QUARTER;
      else if (s == "eighth")
...
TimeDuration TimeDuration::shift(int v) const
      {
      ...
      if ((newValue < 0) || (newValue > V_256TH))
            return TimeDuration();
      return TimeDuration(DurationType(newValue));
      }
...
bool TimeDuration::operator<(const TimeDuration& t) const
      {
      if (t._val < _val)
            return true;
      if (t._val == _val) {
...
Fraction TimeDuration::fraction() const
      {
      int z = 1;
      unsigned n;
      switch(_val) {
...
TimeDuration::TimeDuration(const Fraction& _f)
      {
...
QList<TimeDuration> toDurationList(Fraction l, bool useDottedValues)
      {
      QList<TimeDuration> dList;
      if (useDottedValues) {
            for (TimeDuration d = TimeDuration(TimeDuration::V_LONG); d.isValid() && (l.numerator() != 0);) {
                  d.setDots(2);
