...
//  $Id: rendermidi.cpp 3649 2010-11-01 08:49:38Z wschweer $
//
...
static void playNote(EventMap* events, const Note* note, int channel, int pitch,
   int velo, int onTime, int offTime)
      ...
      SeqEvent ev(ME_NOTEON);
      ev.setChannel(channel);
      ev.setPitch(pitch);
      ev.setVelo(velo);
      ev.setTuning(float(note->tuning()));
      ev.setNote(note);
