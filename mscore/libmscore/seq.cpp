...
void SeqProxy::setRelTempo(float val)
      {
      d->setRelTempo(val);
      }

void SeqProxy::setScoreProxy(ScoreProxy* sp)
      {
      d->setScore(sp->score());
      }

MRect SeqProxy::heartBeat(int* pageIdx, bool* stopped)
      {
      return d->heartBeat(pageIdx, stopped);
      }

void SeqProxy::sendEvent(const SeqEvent& e)
      {
      d->sendEvent(e);
      }

void SeqProxy::seek(int val)
      {
      d->seek(val);
      }
...
MRect Seq::heartBeat(int* pageIdx, bool* stopped)
      {
      MRect r;
      if (state != PLAY) {
            *stopped = true;
