...
xmlTextReaderStartElement(void *ctx, const xmlChar *fullname,
    ...
    xmlTextReaderPtr reader = (XmlTextReader*)ctxt->_private;
...
int XmlTextReader::pushData()
      ...
      xmlTextReaderState oldstate = state;
      state = XML_TEXTREADER_NONE;
      xmlBufferPtr inbuf = input->buffer;

      while (state == XML_TEXTREADER_NONE) {
            if (inbuf->use < cur + CHUNK_SIZE) {
                  /*
	             * Refill the buffer unless we are at the end of the stream
	             */
                  if (mode != XML_TEXTREADER_MODE_EOF) {
                        val = input->read(4096);
                        if ((val == 0) && (inbuf->alloc == XML_BUFFER_ALLOC_IMMUTABLE)) {
		                  if (inbuf->use == cur) {
                              ...
                              mode = XML_TEXTREADER_MODE_EOF;
                              state = oldstate;
                              if ((oldstate != XML_TEXTREADER_START) || (ctxt->myDoc != NULL))
                                    return(val);
		                  }
                        else if (val == 0) {
             ...
             */
            if (inbuf->use >= cur + CHUNK_SIZE) {
                  val = xmlParseChunk(ctxt, (const char *) &inbuf->content[cur], CHUNK_SIZE, 0);
                  cur += CHUNK_SIZE;
                  if ((val != 0) || (ctxt->wellFormed == 0))
                        return(-1);
                  }
            else {
                  s = inbuf->use - cur;
                  val = xmlParseChunk(ctxt, (const char *) &inbuf->content[cur], s, 0);
                  cur += s;
                  if ((val != 0) || (ctxt->wellFormed == 0))
                        return(-1);
                  break;
       ...
       */
      if (mode == XML_TEXTREADER_MODE_INTERACTIVE) {
            if (inbuf->alloc != XML_BUFFER_ALLOC_IMMUTABLE) {
                  if ((cur >= 4096) && (inbuf->use - cur <= CHUNK_SIZE)) {
       ...
       * parser.
       */
      else if (mode == XML_TEXTREADER_MODE_EOF) {
            if (state != XML_TEXTREADER_DONE) {
                  s = inbuf->use - cur;
                  val = xmlParseChunk(ctxt, (const char *) &inbuf->content[cur], s, 1);
                  cur = inbuf->use;
                  state  = XML_TEXTREADER_DONE;
                  if ((val != 0) || (ctxt->wellFormed == 0))
                        return -1;
                  }
            }
...
bool XmlTextReader::read()
      ...
      curnode = NULL;
      if (doc != NULL)
            return xmlTextReaderReadTree(this) == 1;
      if (ctxt == NULL)
            return false;
      if (ctxt->wellFormed != 1)
            return false;

      if (mode == XML_TEXTREADER_MODE_INITIAL) {
            mode = XML_TEXTREADER_MODE_INTERACTIVE;
      	/*
      	 * Initial state
      	 */
            do {
      	      val = pushData();
      			...
      			state = XML_TEXTREADER_ERROR;
      		      return false;
      		      }
      	      } while ((ctxt->node == NULL) && ((mode != XML_TEXTREADER_MODE_EOF) && (state != XML_TEXTREADER_DONE)));

            if (ctxt->node == NULL) {
      	      if (ctxt->myDoc)
                        node = ctxt->myDoc->children;
                  if (node == NULL){
                        mode = XML_TEXTREADER_MODE_ERROR;
      			state = XML_TEXTREADER_ERROR;
      		      return false;
      		      }
                  state = XML_TEXTREADER_ELEMENT;
                  }
            else {
                  if (ctxt->myDoc != NULL) {
                        node = ctxt->myDoc->children;
...
get_next_node:
	...
	if ((preserves == 0) &&
	    (entNr == 0) &&
	    (node->prev != NULL) &&
            (node->prev->type != XML_DTD_NODE) &&
	    (entNr == 0)) {
	    xmlNodePtr tmp = node->prev;
	    if ((tmp->extra & NODE_IS_PRESERVED) == 0) {
		xmlUnlinkNode(tmp);
		xmlTextReaderFreeNode(this, tmp);
	    }
	}
	 ...
	 * Cleanup of the old node
	 */
	if ((oldnode != NULL) && (preserves == 0) &&
	    (entNr == 0) &&
	    (oldnode->type != XML_DTD_NODE) &&
	    ((oldnode->extra & NODE_IS_PRESERVED) == 0) &&
	    (entNr == 0)) {
	    xmlUnlinkNode(oldnode);
	    xmlTextReaderFreeNode(this, oldnode);
	}
	...
	xmlNodePtr tmp = node->last;
	xmlUnlinkNode(tmp);
	xmlTextReaderFreeNode(this, tmp);
    }
    depth--;
    state = XML_TEXTREADER_BACKTRACK;
...
node_found:
    ...
    if ((node != NULL) &&
	 ...
	 (node->type == XML_CDATA_SECTION_NODE))) {
            if (xmlTextReaderExpand(this) == NULL)
	        return -1;
    }
    ...
    if ((node != NULL) &&
	(node->type == XML_ENTITY_REF_NODE) &&
	(ctxt != NULL) && (ctxt->replaceEntities == 1)) {
	/*
	...
	if ((node->children != NULL) &&
	    ...
	    xmlTextReaderEntPush(this, node);
	    node = node->children->children;
	}
    }
    if ((node != NULL) &&
	(node->type == XML_ENTITY_DECL) &&
	(ent != NULL) && (ent->children == node)) {
	node = xmlTextReaderEntPop(this);
	depth++;
        goto get_next_node;
    }
    return(1);
...
XmlTextReader::XmlTextReader()
      {
      memset(this, 0, sizeof(XmlTextReader));   // OOPS?
      }
...
bool XmlTextReader::init(xmlParserInputBufferPtr ip, const char *URI)
      {
      if (!ip)
	      return false;
      doc    = NULL;
      ...
      entNr  = 0;
      input  = ip;
      buffer = xmlBufferCreateSize(100);
      if (buffer == NULL)
            return false;
      ...
      xmlSAXVersion(sax, 2);
      startElement = sax->startElement;
      sax->startElement = xmlTextReaderStartElement;
      endElement = sax->endElement;
      sax->endElement = xmlTextReaderEndElement;
	startElementNs = sax->startElementNs;
	sax->startElementNs = xmlTextReaderStartElementNs;
	endElementNs = sax->endElementNs;
	sax->endElementNs = xmlTextReaderEndElementNs;
      characters = sax->characters;
      sax->characters = xmlTextReaderCharacters;
      sax->ignorableWhitespace = xmlTextReaderCharacters;
      cdataBlock = sax->cdataBlock;
      sax->cdataBlock = xmlTextReaderCDataBlock;

      mode = XML_TEXTREADER_MODE_INITIAL;
      node = NULL;
      curnode = NULL;
      if (input->buffer->use < 4) {
            input->read(4);
            }
      if (input->buffer->use >= 4) {
            ctxt = xmlCreatePushParserCtxt(sax, NULL,
               (const char *) input->buffer->content, 4, URI);
            base = 0;
	      cur = 4;
            }
      else {
            ctxt = xmlCreatePushParserCtxt(sax, NULL, NULL, 0, URI);
            base = 0;
	      cur = 0;
            }
 ...
 * Returns the new xmlTextReaderPtr or NULL in case of error
 */
bool XmlTextReader::init(const char* URI)
      {
...
XmlTextReader::~XmlTextReader()
      {
      if (faketext)
            xmlFreeNode(faketext);
      if (ctxt) {
            if (dict == ctxt->dict)
                  dict = 0;
            if (ctxt->myDoc) {
                  if (preserve == 0)
                        xmlTextReaderFreeDoc(this, ctxt->myDoc);
                  ctxt->myDoc = 0;
                  }
            if (ctxt->vctxt.vstateTab && (ctxt->vctxt.vstateMax > 0)) {
                  xmlFree(ctxt->vctxt.vstateTab);
                  ctxt->vctxt.vstateTab = 0;
                  ctxt->vctxt.vstateMax = 0;
                  }
            if (allocs & XML_TEXTREADER_CTXT)
                  xmlFreeParserCtxt(ctxt);
            }
      if (sax)
            xmlFree(sax);
      if (input  && (allocs & XML_TEXTREADER_INPUT))
            delete input;
      if (buffer)
            xmlBufferFree(buffer);
      if (entTab)
...
bool XmlTextReader::moveToNextAttribute()
	...
	return false;
    if (curnode == NULL)
	return(xmlTextReaderMoveToFirstAttribute(this));

    if (curnode->type == XML_NAMESPACE_DECL) {
	xmlNsPtr ns = (xmlNsPtr) curnode;
	if (ns->next != NULL) {
	    curnode = (xmlNodePtr) ns->next;
	    return true;
	}
	if (node->properties != NULL) {
	    curnode = (xmlNodePtr) node->properties;
	    return true;
	}
	return false;
    } else if ((curnode->type == XML_ATTRIBUTE_NODE) &&
	       (curnode->next != NULL)) {
...
int XmlTextReader::attributeCount() const
      ...
      xmlNodePtr xnode = curnode ? curnode : node;

      if (xnode->type != XML_ELEMENT_NODE)
	      return 0;
      if ((state == XML_TEXTREADER_END) || (state == XML_TEXTREADER_BACKTRACK))
	      return 0;
      int ret = 0;
...
int XmlTextReader::nodeType() const
      ...
      switch (xnode->type) {
		...
		if (xmlNodeGetSpacePreserve(node))
...
bool XmlTextReader::isEmptyElement() const
      ...
      if (node->children)
	      return true;
      if (state == XML_TEXTREADER_END)
	      return true;
      if (doc)
            return false;
      return (node->extra & NODE_IS_EMPTY);
      }
 ...
 */
const xmlChar* XmlTextReader::name() const
      {
      xmlNodePtr xnode;
      ...
      switch (xnode->type) {
        ...
        case XML_ATTRIBUTE_NODE:
	    if ((xnode->ns == NULL) ||
		(xnode->ns->prefix == NULL))
		return(xnode->name);
	    return xmlDictQLookup(dict, xnode->ns->prefix, node->name);
        case XML_TEXT_NODE:
	    return xmlDictLookup(dict, BAD_CAST "#text", -1);
        case XML_CDATA_SECTION_NODE:
	    return xmlDictLookup(dict, BAD_CAST "#cdata-section", -1);
        case XML_ENTITY_NODE:
        case XML_ENTITY_REF_NODE:
	    return _CONSTSTR(xnode->name);
        case XML_PI_NODE:
	    return(_CONSTSTR(xnode->name));
        case XML_COMMENT_NODE:
	    return(_CONSTSTR(BAD_CAST "#comment"));
        case XML_DOCUMENT_NODE:
        case XML_HTML_DOCUMENT_NODE:
	    return(_CONSTSTR(BAD_CAST "#document"));
        case XML_DOCUMENT_FRAG_NODE:
	    return(_CONSTSTR(BAD_CAST "#document-fragment"));
        case XML_NOTATION_NODE:
	    return(_CONSTSTR(xnode->name));
        case XML_DOCUMENT_TYPE_NODE:
        case XML_DTD_NODE:
	    ...
	    if (ns->prefix == NULL)
		return(_CONSTSTR(BAD_CAST "xmlns"));
	    return xmlDictQLookup(dict, BAD_CAST "xmlns", ns->prefix);
	}
 ...
 */
int XmlTextReader::getDepth() const
      {
      if (!node)
	      return 0;

      if (curnode) {
            if ((curnode->type == XML_ATTRIBUTE_NODE) || (curnode->type == XML_NAMESPACE_DECL))
                  return depth + 1;
            return depth + 2;
            }
      return depth;
 ...
 */
const xmlChar* XmlTextReader::value() const
      {
      if (!node)
            return 0;
      xmlNodePtr xnode = curnode ? curnode : node;
      ...
      switch (xnode->type) {
		...
		if (buffer == NULL) {
	        ...
	        buffer->use = 0;
	        xmlNodeBufGetContent(buffer, xnode);
 ...
 */
int XmlTextReader::setup(xmlParserInputBufferPtr input, const char *URL,
   const char *encoding, int options)
      {
      XmlTextReader* reader = this;
    /*
...
xmlReaderForDoc(const xmlChar * cur, const char *URL, const char *encoding, int options)
      ...
      return new XmlTextReader((const char *) cur, len, URL, encoding, options);
      }
 ...
 * Returns the new reader or NULL in case of error.
 */
bool XmlTextReader::init(const char *filename, const char *encoding, int options)
      {
      bool rv = init(filename);
      setup(NULL, NULL, encoding, options);
      return rv;
      }
 ...
 * Returns the new reader or NULL in case of error.
 */
XmlTextReader::XmlTextReader(const char *buffer, int size, const char *URL,
   const char *encoding, int options)
      {
      memset(this, 0, sizeof(XmlTextReader));   // OOPS?
      ...
      if (buf) {
            ...
            allocs |= XML_TEXTREADER_INPUT;
            setup(NULL, URL, encoding, options);
            }
      }
...
xmlReaderForFd(int fd, const char *URL, const char *encoding, int options)
{
    ...
    reader->allocs |= XML_TEXTREADER_INPUT;
    reader->setup(NULL, URL, encoding, options);
#endif
    return (reader);
}
                 ...
                 const char *encoding, int options)
{
    ...
    if (input == NULL)
        return (-1);
    return reader->setup(input, filename, encoding, options);
}
                   ...
                   const char *URL, const char *encoding, int options)
{
    ...
    return reader->setup(input, URL, encoding, options);
}
