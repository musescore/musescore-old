 ...
 *         in case of error.
 */
int XmlParserInputBuffer::push(int len, const char *buf)
      {
      XmlParserInputBuffer* in = this;
 ...
 *         in case of error.
 */
int XmlParserInputBuffer::grow(int len)
      {
      XmlParserInputBuffer* in = this;
...
int XmlParserInputBuffer::read(int len)
      {
      if (error)
            return -1;
      if (readcallback)
            return grow(len);
      else if (buffer && (buffer->alloc == XML_BUFFER_ALLOC_IMMUTABLE))
            return 0;
      else
            return -1;
      }
