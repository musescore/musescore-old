...
#if defined(MACOS9)
/* Not implemented */
fluid_thread_t* new_fluid_thread(fluid_thread_func_t func, void* data, int detach) { return 0; }
...
fluid_thread_t* new_fluid_thread(fluid_thread_func_t func, void* data, int detach)
{
  ...
  thread->thread = CreateThread(0, 0, fluid_thread_start, (LPVOID) thread,
				0, &thread->thread_id);
  if (thread->thread == 0) {
...
int fluid_thread_join(fluid_thread_t* thread)
{
  ...
  if (thread->pthread != 0) {
    err = pthread_join(thread->pthread, 0);
  }
  return (err == 0)? FLUID_OK : FLUID_FAILED;
}
