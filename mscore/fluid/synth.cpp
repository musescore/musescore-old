...
{
  fluid_settings_register_str(settings, "synth.verbose", "no", 0, 0, 0);
  fluid_settings_register_str(settings, "synth.dump", "no", 0, 0, 0);
  fluid_settings_register_str(settings, "synth.reverb.active", "yes", 0, 0, 0);
  fluid_settings_register_str(settings, "synth.chorus.active", "yes", 0, 0, 0);
  fluid_settings_register_str(settings, "synth.ladspa.active", "no", 0, 0, 0);
...
new_fluid_synth(fluid_settings_t *settings)
{
  ...
  synth->fx_left_ubuf = 0;
  synth->fx_right_ubuf = 0;
  ...
  if ((synth->left_buf == 0) || (synth->right_buf == 0) ||
      (synth->left_ubuf == 0) || (synth->right_ubuf == 0)) {
    FLUID_LOG(FLUID_ERR, "Out of memory");
    goto error_recovery;
  }
  ...
  for (i = 0; i < synth->nbuf; i++) {
    ...
    if ((synth->left_ubuf[i] == 0) || (synth->right_ubuf[i] == 0)) {
      FLUID_LOG(FLUID_ERR, "Out of memory");
      goto error_recovery;
    }
  ...
  if ((synth->fx_left_buf == 0) || (synth->fx_left_ubuf == 0) ||
      (synth->fx_right_buf == 0) || (synth->fx_right_ubuf == 0)) {
    FLUID_LOG(FLUID_ERR, "Out of memory");
    goto error_recovery;
  }
  ...
  for (i = 0; i < synth->effects_channels; i++) {
    ...
    if ((synth->fx_left_ubuf[i] == 0) || (synth->fx_right_ubuf[i] == 0)) {
      FLUID_LOG(FLUID_ERR, "Out of memory");
      goto error_recovery;
    }
...
delete_fluid_synth(fluid_synth_t* synth)
{
  ...
  if (synth->left_ubuf != 0) {
    for (i = 0; i < synth->nbuf; i++) {
      if (synth->left_ubuf[i] != 0) {
  ...
  if (synth->right_ubuf != 0) {
    for (i = 0; i < synth->nbuf; i++) {
      if (synth->right_ubuf[i] != 0) {
  ...
  if (synth->fx_left_ubuf != 0) {
    for (i = 0; i < 2; i++) {
      if (synth->fx_left_ubuf[i] != 0) {
  ...
  if (synth->fx_right_ubuf != 0) {
    for (i = 0; i < 2; i++) {
      if (synth->fx_right_ubuf[i] != 0) {
...
{
  int i, k;
  fluid_voice_t* voice = 0;
  fluid_channel_t* channel = 0;
 ...
 */
int
fluid_synth_sfload(fluid_synth_t* synth, const QString& filename, int reset_presets)
{
  fluid_sfont_t* sfont;
  fluid_list_t* list;
  fluid_sfloader_t* loader;
  ...
  FLUID_LOG(FLUID_ERR, "Failed to load SoundFont \"%s\"", filename.toLatin1().data());
  return -1;
}
...
fluid_synth_create_tuning(fluid_synth_t* synth, int bank, int prog, char* name)
{
  ...
  if ((fluid_tuning_get_name(synth->tuning[bank][prog]) == 0)
...
int fluid_synth_reset_tuning(fluid_synth_t* synth, int chan)
{
  ...
  fluid_channel_set_tuning(synth->channel[chan], 0);
