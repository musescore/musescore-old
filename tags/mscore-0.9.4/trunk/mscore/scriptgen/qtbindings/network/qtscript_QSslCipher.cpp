...
QScriptValue qtscript_create_QSslCipher_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 12; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSslCipher_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSslCipher_static_call, proto, function_lengths[0]);
