...
QScriptValue qtscript_create_QMenuBar_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 15; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QMenuBar_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QMenuBar_static_call, proto, function_lengths[0]);
