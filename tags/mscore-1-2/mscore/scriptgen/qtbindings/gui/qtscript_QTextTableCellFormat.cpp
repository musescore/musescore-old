...
QScriptValue qtscript_create_QTextTableCellFormat_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 10; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QTextTableCellFormat_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QTextTableCellFormat_static_call, proto, function_lengths[0]);
