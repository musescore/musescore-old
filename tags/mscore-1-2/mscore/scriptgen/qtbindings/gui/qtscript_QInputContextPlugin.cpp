...
QScriptValue qtscript_create_QInputContextPlugin_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 6; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QInputContextPlugin_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QInputContextPlugin_static_call, proto, function_lengths[0]);
