...
QScriptValue qtscript_create_QPalette_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 35; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QPalette_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QPalette_static_call, proto, function_lengths[0]);
