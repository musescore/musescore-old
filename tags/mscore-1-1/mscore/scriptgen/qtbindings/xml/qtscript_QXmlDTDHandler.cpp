...
QScriptValue qtscript_create_QXmlDTDHandler_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 4; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QXmlDTDHandler_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QXmlDTDHandler_static_call, proto, function_lengths[0]);
