...
static QScriptValue qtscript_QNetworkProxyFactory_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (!_q_self) {
        return context->throwError(QScriptContext::TypeError,
            ...
            .arg(qtscript_QNetworkProxyFactory_function_names[_id+4]));
    }
    ...
    return qtscript_QNetworkProxyFactory_throw_ambiguity_error_helper(context,
        qtscript_QNetworkProxyFactory_function_names[_id+4],
        qtscript_QNetworkProxyFactory_function_signatures[_id+4]);
}
...
QScriptValue qtscript_create_QNetworkProxyFactory_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 2; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QNetworkProxyFactory_prototype_call, function_lengths[i+4]);
        fun.setData(QScriptValue(engine, uint(0xBABE0000 + i)));
        proto.setProperty(QString::fromLatin1(qtscript_QNetworkProxyFactory_function_names[i+4]),
            fun, QScriptValue::SkipInEnumeration);
    }

    engine->setDefaultPrototype(qMetaTypeId<QNetworkProxyFactory*>(), proto);

    QScriptValue ctor = engine->newFunction(qtscript_QNetworkProxyFactory_static_call, proto, function_lengths[0]);
