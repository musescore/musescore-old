...
QScriptValue qtscript_create_QSslCertificate_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 18; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSslCertificate_prototype_call, function_lengths[i+4]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSslCertificate_static_call, proto, function_lengths[0]);
