...
QScriptValue qtscript_create_QAccessibleEvent_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 4; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QAccessibleEvent_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QAccessibleEvent_static_call, proto, function_lengths[0]);
