...
static QScriptValue qtscript_QPolygonF_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 1) {
        ...
        QPointF _q_result = _q_self->at(_q_arg0);
    ...
    case 11:
    if (context->argumentCount() == 0) {
        QPointF _q_result = _q_self->first();
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
    ...
    case 16:
    if (context->argumentCount() == 0) {
        QPointF _q_result = _q_self->last();
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
...
QScriptValue qtscript_create_QPolygonF_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 42; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QPolygonF_prototype_call, function_lengths[i+2]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QPolygonF_static_call, proto, function_lengths[0]);
