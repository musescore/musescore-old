...
QScriptValue qtscript_create_QSplitter_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 19; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSplitter_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSplitter_static_call, proto, function_lengths[0]);
