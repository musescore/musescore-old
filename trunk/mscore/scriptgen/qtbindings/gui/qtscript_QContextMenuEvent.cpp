...
static QString qtscript_QContextMenuEvent_Reason_toStringHelper(QContextMenuEvent::Reason value)
{
    if ((value >= QContextMenuEvent::Mouse) && (value <= QContextMenuEvent::Other))
        return qtscript_QContextMenuEvent_Reason_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QContextMenuEvent_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 8; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QContextMenuEvent_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QContextMenuEvent_static_call, proto, function_lengths[0]);
