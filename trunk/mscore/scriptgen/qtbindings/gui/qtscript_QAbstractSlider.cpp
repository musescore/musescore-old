...
static QString qtscript_QAbstractSlider_SliderAction_toStringHelper(QAbstractSlider::SliderAction value)
{
    if ((value >= QAbstractSlider::SliderNoAction) && (value <= QAbstractSlider::SliderMove))
        return qtscript_QAbstractSlider_SliderAction_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QAbstractSlider_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 3; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QAbstractSlider_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QAbstractSlider_static_call, proto, function_lengths[0]);
