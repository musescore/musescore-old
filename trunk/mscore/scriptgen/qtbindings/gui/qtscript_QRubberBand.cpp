...
static QString qtscript_QRubberBand_Shape_toStringHelper(QRubberBand::Shape value)
{
    if ((value >= QRubberBand::Line) && (value <= QRubberBand::Rectangle))
        return qtscript_QRubberBand_Shape_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QRubberBand_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 5; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QRubberBand_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QRubberBand_static_call, proto, function_lengths[0]);
