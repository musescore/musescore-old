...
static QString qtscript_QWorkspace_WindowOrder_toStringHelper(QWorkspace::WindowOrder value)
{
    if ((value >= QWorkspace::CreationOrder) && (value <= QWorkspace::StackingOrder))
        return qtscript_QWorkspace_WindowOrder_keys[static_cast<int>(value)];
    return QString();
}
...
QScriptValue qtscript_create_QWorkspace_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 5; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QWorkspace_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QWorkspace_static_call, proto, function_lengths[0]);
