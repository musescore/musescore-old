...
static const char * const qtscript_QGraphicsPathItem_function_signatures[] = {
    "QGraphicsItem parent, QGraphicsScene scene\nQPainterPath path, QGraphicsItem parent, QGraphicsScene scene"
    // static
    // prototype
    , ""
    , "QPainterPath path"
...
static QScriptValue qtscript_QGraphicsPathItem_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 0) {
        ...
        QtScriptShell_QGraphicsPathItem* _q_cpp_result = new QtScriptShell_QGraphicsPathItem(_q_arg0, _q_arg1, _q_arg2);
        QScriptValue _q_result = context->engine()->newVariant(context->thisObject(), qVariantFromValue((QGraphicsPathItem*)_q_cpp_result));
        _q_cpp_result->__qtscript_self = _q_result;
        return _q_result;
    }
...
QScriptValue qtscript_create_QGraphicsPathItem_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 3; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QGraphicsPathItem_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QGraphicsPathItem_static_call, proto, function_lengths[0]);
