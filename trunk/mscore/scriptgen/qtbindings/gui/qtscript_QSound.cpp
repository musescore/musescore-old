...
static QScriptValue qtscript_QSound_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (!_q_self) {
        return context->throwError(QScriptContext::TypeError,
            ...
            .arg(qtscript_QSound_function_names[_id+1]));
    }
...
QScriptValue qtscript_create_QSound_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 6; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSound_prototype_call, function_lengths[i+3]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSound_static_call, proto, function_lengths[0]);
