...
QScriptValue qtscript_create_QMenu_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 20; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QMenu_prototype_call, function_lengths[i+2]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QMenu_static_call, proto, function_lengths[0]);
