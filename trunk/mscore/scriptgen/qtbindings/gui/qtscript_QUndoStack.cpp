...
QScriptValue qtscript_create_QUndoStack_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 17; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QUndoStack_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QUndoStack_static_call, proto, function_lengths[0]);
