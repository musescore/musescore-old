...
QScriptValue qtscript_create_QTextFragment_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 10; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QTextFragment_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QTextFragment_static_call, proto, function_lengths[0]);
