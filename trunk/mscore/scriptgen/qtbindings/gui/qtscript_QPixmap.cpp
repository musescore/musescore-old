...
static QScriptValue qtscript_QPixmap_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    return qtscript_QPixmap_throw_ambiguity_error_helper(context,
        qtscript_QPixmap_function_names[_id+6],
        qtscript_QPixmap_function_signatures[_id+6]);
}
...
QScriptValue qtscript_create_QPixmap_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 26; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QPixmap_prototype_call, function_lengths[i+6]);
        fun.setData(QScriptValue(engine, uint(0xBABE0000 + i)));
        proto.setProperty(QString::fromLatin1(qtscript_QPixmap_function_names[i+6]),
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QPixmap_static_call, proto, function_lengths[0]);
