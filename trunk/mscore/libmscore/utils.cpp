...
//  $Id: utils.cpp 3504 2010-09-24 12:10:50Z lasconic $
//
...
Segment* Score::tick2segment(int tick, bool first, SegmentTypes st) const
      ...
      for (Segment* segment = m->first(st); segment;) {
            ...
            int t2 = nsegment ? nsegment->tick() : 0x7ffffff;     // INT_MAX;
 ...
 *  Determines whether the output may include qreal sharbs or flats (Abb)
 ...
 * @param useDoubleSharpsFlats
 *  Determines whether the output may include qreal sharps or flats (Abb)
 *  or should use an enharmonic pitch (Abb = G).
 *
 * @return
