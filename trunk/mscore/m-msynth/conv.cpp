...
void fluid_conversion_config()
      ...
      for (int i = 0; i < FLUID_CB_AMP_SIZE; i++)
            fluid_cb2amp_tab[i] = (float) pow(10.0, (float) i / -200.0);
      ...
      for (int i = 0; i < FLUID_ATTEN_AMP_SIZE; i++)
            fluid_atten2amp_tab[i] = (float) pow(10.0, (float) i / FLUID_ATTEN_POWER_FACTOR);
      ...
      for (int i = 1; i < 127; i++) {
            float x = -20.0 / 96.0 * log((i * i) / (127.0 * 127.0)) / log(10.0);
            fluid_convex_tab[i] = (float) (1.0 - x);
            fluid_concave_tab[127 - i] = (float) x;
            }

      /* initialize the pan conversion table */
      float x = M_PI / 2.0 / (FLUID_PAN_SIZE - 1.0);
...
float fluid_tc2sec_delay(float tc)
      ...
      if (tc > 5000.0f)
            tc = (float) 5000.0f;
      return (float) pow(2.0, (float) tc / 1200.0);
      }
...
float fluid_act2hz(float c)
      {
      return (float) (8.176 * pow(2.0, (float) c / 1200.0));
      }
